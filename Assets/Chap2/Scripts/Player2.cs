...
public class Player2 : MonoBehaviour
{
    ...
    private bool isStunnedBoss2 = false;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (collision.CompareTag("Boss2MouthPoint"))
        {
    ...
    {
        if (isStunnedBoss2) return; // 스턴 상태일 때 움직임을 처리하지 않음
    ...
    {
        if (isStunnedBoss2) return; // 스턴 상태일 때 공격 처리를 하지 않음
